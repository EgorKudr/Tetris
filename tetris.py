...
lightcolors = ((30, 30, 255),   # светло-синий
...
brd_color, bg_color, txt_color, title_color, info_color, txt_color1 = white, black, white, colors[3], colors[1], colors[2]
...
def runTetris():
    ...
    while True:
        ...
        for event in pg.event.get():
            if event.type == KEYUP:
            ...
            elif event.type == KEYDOWN:     # перемещение фигуры вправо и влево
                ...
                elif event.key == K_UP:     # поворачиваем фигуру, если есть место
                    fallingFig['rotation'] = (fallingFig['rotation'] + 1) % len(figures[fallingFig['shape']])
                    if not checkPos(cup, fallingFig):
                        fallingFig['rotation'] = (fallingFig['rotation'] - 1) % len(figures[fallingFig['shape']])

                elif event.key == K_DOWN:    # ускоряем падение фигуры
                    going_down = True
                    if checkPos(cup, fallingFig, adjY=1):
                        fallingFig['y'] += 1
                    last_move_down = time.time()

                elif event.key == K_RETURN:    # мгновенный сброс вниз
                    going_down = False
                    going_left = False
                    going_right = False
        ...
        display_surf.fill(bg_color)         # рисуем окно игры со всеми надписями
        drawTitle()
        gamecup(cup)
...
def calcSpeed(points):          # вычисляет уровень
...
def emptycup():         # создает пустой стакан
    cup = []
...
def checkPos(cup, fig, adjX=0, adjY=0):     # проверяет, находится ли фигура в границах стакана,
    for x in range(fig_w):                  # не сталкиваясь с другими фигурами^
        for y in range(fig_h):
...
def isCompleted(cup, y):            # проверяем наличие полностью заполненных рядов
...
def clearCompleted(cup):            # Удаление заполенных рядов и сдвиг верхних рядов вниз
    removed_lines = 0
    y = cup_h - 1
    while y >= 0:
...
def gamecup(cup):   # граница игрового поля-стакана
    pg.draw.rect(display_surf, brd_color, (side_margin - 4, top_margin - 4, (cup_w * block) + 8, (cup_h * block) + 8), 5)

    pg.draw.rect(display_surf, bg_color, (side_margin, top_margin, block * cup_w, block * cup_h))   # фон игрового поля
    for x in range(cup_w):
...
def drawTitle():
    ...
    titleRect.topleft = (window_w - 383, 10)
    display_surf.blit(titleSurf, titleRect)
...
def drawnextFig(fig):   # превью следующей фигуры
    nextSurf = basic_font.render('Следующая:', True, txt_color1)
